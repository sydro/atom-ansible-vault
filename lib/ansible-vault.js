...
atom.config.onDidChange('ansible-vault.vault_automatic_de_and_encrypt', function(arg) {
  if (!atom.config.get("ansible-vault.vault_automatic_de_and_encrypt")) {
    ...
    if (atom.config.get('ansible-vault.vault_password_file_flag')) {
      ...
      obj_utils.notifyMessage(3,"","ERROR: Automatic de- and encryption is available only with Vault password file enabled!");
      //atom.config.set("ansible-vault.vault_automatic_de_and_encrypt", false);
    }
  }
  console.log(arg);
});
    ...
    if (atom.config.get('ansible-vault.vault_password_file_flag')) {
      ...
      if (this.modalPanel.isVisible()) {
        ...
        try {
          ...
          this.notifyMessage(3,"","ERROR: ansible-vault not found using path in package settings")
          this.reset(this.ansibleVaultView);
          return
        }
