...
export default class AnsibleVaultUtils {
  ...
  findConfig(vault_filepath) {
    ...
    const dirname_vaultfile = path.dirname(vault_filepath);
    let ansible_cfg_file = listFiles(dirname_vaultfile, ['cfg']).map(function(el) { if (el.includes('ansible.cfg')) return el ; } );
    ...
    if (typeof ansible_cfg_file[0] == "undefined") return [];
