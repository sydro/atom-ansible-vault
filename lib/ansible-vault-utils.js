...
export default class AnsibleVaultUtils {
  ...
  findConfig(vault_filepath) {
    ...
    do {
      ansible_cfg_file = listFiles(dirname_vaultfile, ["cfg"]).map(function(
        el
      ) {
      ...
      ansible_cfg_file.length <= 0 &&
      dirname_vaultfile.includes(project_path)
    );
    ...
    if (atom.config.get("ansible-vault.vault_password_file_forcing")) {
      ...
      if (atom.config.get("ansible-vault.vault_password_file_flag")) {
        ...
        if (ansible_cfg_file.length <= 0) {
          password_file = atom.config.get(
            "ansible-vault.vault_password_file_path"
          );
        } else {
          let ansible_cfg = ini.parse(
            fs.readFileSync(ansible_cfg_file[0], "utf-8")
          );
  ...
  ExecAction(editor, vault_filepath, action, password, status = "") {
    ...
    if (status != "auto")
      ...
      father.VaultFile(editor, vault_filepath, action, password_file, "auto");
  }
  ...
  EnableAuto() {
    ...
    if (atom.config.get("ansible-vault.vault_automatic_de_and_encrypt")) {
      ...
      this.promise_add_editor = atom.workspace.onDidAddTextEditor(function(
        event
      ) {
        if (!atom.config.get("ansible-vault.vault_automatic_de_and_encrypt")) {
        ...
        if (event.textEditor.getText().substring(0, 14) == "$ANSIBLE_VAULT") {
          let promise_editor = event.textEditor.onDidChange(function() {
            ...
            let promise_close_editor = actived_editor_pane.onWillRemoveItem(
              function(event) {
